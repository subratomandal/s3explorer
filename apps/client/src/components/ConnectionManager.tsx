...
import { Plus, Trash2, Check, Server, Edit2, ChevronRight, Globe, Shield } from 'lucide-react';
...
export function ConnectionManager({ isOpen, onClose, onConnectionChange }: ConnectionManagerProps) {
  ...
  function startEdit(e: React.MouseEvent, conn: Connection) {
    ...
    setForm({
      ...
      accessKey: '', // Security: Don't show existing keys
      secretKey: '',
      region: conn.region,
      ...
      <div className="min-h-[400px] flex flex-col">
        ...
        {view === 'list' ? (
          <>
              ...
              {loading ? (
                ...
                connections.map((conn) => (
                  <div
                    ...
                    className={`group relative flex items-center justify-between p-4 rounded-xl border transition-all cursor-pointer ${conn.isActive
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${conn.isActive ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-800 text-gray-500'
                        }`}>
                        <Server className="w-5 h-5" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className={`font-medium ${conn.isActive ? 'text-white' : 'text-gray-300'}`}>
                            {conn.name}
                          </h3>
                          {conn.isActive && (
                        ...
                        <p className="text-xs text-gray-500 mt-0.5 font-mono">{conn.endpoint}</p>
                      </div>
                    </div>
                    ...
                    {!conn.isActive && (
                      <ChevronRight className="w-4 h-4 text-gray-600 absolute right-4 group-hover:opacity-0 transition-opacity" />
                    )}
                  </div>
                ))
              )}
            ...
            <div className="mt-6 pt-4 border-t border-gray-800">
              <button
                ...
                className="w-full py-3 px-4 rounded-xl border border-dashed border-gray-700 text-gray-400 hover:text-white hover:border-gray-500 hover:bg-gray-800/50 transition-all flex items-center justify-center gap-2 font-medium text-sm"
              >
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="space-y-5 flex-1 overflow-y-auto px-1">
              ...
              <div className="grid grid-cols-2 gap-4 items-end">
                ...
                <label className="flex items-center gap-3 p-2.5 cursor-pointer group">
                  <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${form.forcePathStyle
            ...
            <div className="mt-8 pt-4 flex items-center justify-end gap-3 border-t border-gray-800">
              <button
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-lg text-sm font-medium text-white bg-purple-600 hover:bg-purple-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-[0_0_15px_-5px_rgba(147,51,234,0.5)]"
              >
